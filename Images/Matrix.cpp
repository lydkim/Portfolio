...
int Matrix_row(const Matrix* mat, const int* ptr) {
  ...
  row = index / width;
  //address of beginning of data + 2 = pointer
...
int* Matrix_at(Matrix* mat, int row, int column) {
  int element = (Matrix_width(mat) * row) + column;
  int *ptr = &(mat -> data[element]);
  //matrix object and then getting the data array
  //indexing in the data array
  return ptr;
}
...
void Matrix_fill_border(Matrix* mat, int value) {
  ...
  //when row is 0 and when row = height - 1
  //when column is 0 and when column = width - 1
}
