...
void compute_energy_matrix(const Image* img, Matrix* energy) {
  ...
  for (int r = 1; r < (Matrix_height(energy) - 1); ++r) {
    for (int c = 1; c < (Matrix_width(energy) - 1); ++c) {
      ...
      currentEnergy = squared_difference(Image_get_pixel(img, r-1, c), 
      Image_get_pixel(img, r+1, c)) + squared_difference(Image_get_pixel(img, r, c-1),
      Image_get_pixel(img, r, c+1));
      *Matrix_at(energy, r, c) = currentEnergy;
    ...
    for (int c = 0; c < Matrix_width(cost); ++c) {
      if (c == 0) {
        *Matrix_at(cost, r, c) = *Matrix_at(energy, r, c)+ 
        Matrix_min_value_in_row(cost, r-1, c, c+1);
      }
      else if(c == Matrix_width(cost) - 1) {
        *Matrix_at(cost, r, c) = *Matrix_at(energy, r, c) + 
        Matrix_min_value_in_row(cost, r-1, c-1, c);
      }
      else {
        *Matrix_at(cost, r, c) = *Matrix_at(energy, r, c) + 
...
void find_minimal_vertical_seam(const Matrix* cost, int seam[]) {
  ...
  for (int r = (Matrix_height(cost)-2); r >= 0; r--) {
    //when column 0 all the way to the end
    if (seam[r+1] == 0) {
    seam[r] = Matrix_column_of_min_value_in_row(cost, r, (seam[r+1]), (seam[r+1]+2));
    }
    //when column is all the way to the right
    else if (seam[r+1] == Matrix_width(cost)-1) {
      seam[r] = Matrix_column_of_min_value_in_row(cost, r, (seam[r+1]-1), (seam[r+1]+1));
    }
     //middle
    else {
...
void remove_vertical_seam(Image *img, const int seam[]) {
  ...
  for (int r = 0; r < Image_height(img); ++r) {
    for (int c = 0; c < Image_width(img); ++c) {
      if(c < seam[r]) {
        Image_set_pixel(imgSmall, r, c, Image_get_pixel(img, r, c));
      }
      else if (c > seam[r]) {
        Image_set_pixel(imgSmall, r, c-1, Image_get_pixel(img, r, c));
      }
    }
  }
  *img = *imgSmall;
...
void seam_carve_width(Image *img, int newWidth) {
  ...
  //do we need to add heights and widths 
