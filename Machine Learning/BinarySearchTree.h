...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    ...
    //false if the max of the left is greater than the node
    if (node->left != nullptr && less(node->datum, 
    max_element_impl(node->left)->datum)) {
      return false;
    }
    if (node->right != nullptr && less(min_element_impl(node->right)->datum, 
    node->datum)) {
      return false;
    }
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    //root <= to value go right, otherwise go left
